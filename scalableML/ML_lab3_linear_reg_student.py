...
def parsePoint(line):
    ...
    """
    points = line.split(',')
    return LabeledPoint(points[0], points[1:])
...
def calcRMSE(labelsAndPreds):
    ...
    """
    mean = labelsAndPreds.map(lambda (x,y): squaredError(x, y)).mean()
    return mean**0.5
...
def gradientSummand(weights, lp):
    ...
    """
    array = (weights.dot(lp.features) - lp.label)*lp.features
    return array
...
def twoWayInteractions(lp):
    ...
    """
    tuples = list(itertools.product(lp.features, repeat=2))
    twoWayInt = []
    for i in tuples:
        twoWayInt = twoWayInt + [reduce(lambda x,y: x*y, i)]
    features = np.hstack((lp.features, twoWayInt))
